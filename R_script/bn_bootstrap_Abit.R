  ...
  
{
 ...
 ##### Construction du modele avant bootstrap 
  
  Drivers=c("Scenario")
  Year=c("Time")
  States=c("Veg","Dist")
  Impacts_Ab=c("Ab_MooseHunting","Ab_Fishing","Ab_trapping","Ab_Ressourcement","Ab_Blueberry")
  Impacts_Qu=c("Qu_Fishing","Qu_trapping")
  ...
  Veg.lv=c("VegA1","VegA2","VegA3","VegA4","VegA5","VegA6","VegA7")
  scenario.lv=unique(Vege.prob.table$ScenarioCCFor)
  
  alt_abit=tbl_df(read.csv("alt_km2_piko.csv",header=TRUE, sep=";"))
  alt_OB=tbl_df(read.csv("alt_km2_ouje.csv",header=TRUE, sep=";"))
  # Pour Abitibiwinni, cluster 1 = N, cluster 2=S
  Dist_freq_Abi=alt_abit %>%
  ...
  Dist.prob <- matrix(c(Dist_freq_Abi$freq[1], Dist_freq_Abi$freq[2]), ncol=2,dimnames = list(c(""), Dist.lv))
  Dist.prob
  ...
  ScenarioCCForpiko=paste(datapiko$scenario,datapiko$treatment)
  datapiko=bind_cols(datapiko,ScenarioCCFor=ScenarioCCForpiko)
  
 ntrials=1000
                          
  
boot_Abit=list()
boot_Abit_qtChg=list()
  for (l in 1:ntrials)
  {
    print(l)
...
prob_pre[sample(1:5,nrow(prob_pre),replace=TRUE),] 


boot1=filter(data,replicate==repl[1])%>%
         mutate(repl_boot=1)

fucktoute=bind_rows(filter(data,replicate==repl[1])%>%
                      mutate(repl_boot=1),
                    filter(data,replicate==repl[2])%>%
                      mutate(repl_boot=2),
                    filter(data,replicate==repl[3])%>%
                      mutate(repl_boot=3),
                    filter(data,replicate==repl[4])%>%
                      mutate(repl_boot=4),
                    filter(data,replicate==repl[5])%>%
                      mutate(repl_boot=5))
    ...
    for (j in 1:length(Scenario.lv))
    {
      ...
      for (i in Time.lv)
      {
        prob_veg=pkgcond::suppress_messages( prob_veg_landis(time=i,scenario=Scenario.lv[j],method="sample",FN="Abitibiwinni"))
    ...
    tabveg7=Vege.prob.table_1_6 %>%
      filter(cluster=="VegA2")%>%
      mutate(probA7=0)
    
    ### définition des récoltes de feux selon les scénarios 
    tabveg7[tabveg7$ScenarioCCFor %in% x2,]$probA7=tabveg7[tabveg7$ScenarioCCFor %in% x2,]$prob  
    tabveg7[tabveg7$ScenarioCCFor %in% x15,]$probA7=tabveg7[tabveg7$ScenarioCCFor %in% x15,]$prob*0.75  
    tabveg7[tabveg7$ScenarioCCFor %in% x1,]$probA7=tabveg7[tabveg7$ScenarioCCFor %in% x1,]$prob*0.5  
    tabveg7[tabveg7$ScenarioCCFor %in% x05,]$probA7=tabveg7[tabveg7$ScenarioCCFor %in% x05,]$prob*0.25  
    
    ### Arranger tabveg7 pour qu'il ait la meme structure que Vege.prob.table
    tabveg7_test=tabveg7 %>%
      mutate(cluster="VegA7",prob=probA7)%>%
    ...
    data_experts_Abit=
      data_experts %>%
      filter(Communaute=="Abitibiwinni")
    unique(data_experts_Abit$Question)
    
    Im.prob_list=list()
    for (i in 1: length(unique(data_experts_Abit$Question)))
    {
      
      Question_i=unique(data_experts_Abit$Question)[i]
      prob_Imp=pkgcond::suppress_messages(prob_experts(qst=Question_i,method="mean",FN="Abitibiwinni"))
      
      dimname_i=list(Im=Im.lv,Veg=Veg.lv,Dist=Dist.lv)
      names(dimname_i)[1]=unique(data_experts_Abit$Im_BN)[i]
      
      unique(data_experts_Abit$Im_BN)[i]
    ...
    names(Im.prob_list)=paste(unique(data_experts_Abit$Im_BN),".prob",sep="")
    names(Im.prob_list)
    ######### Sortir l'objet tpc
    ...
    bn_Abit=custom.fit(dag,tpc)
    
    ######### Convertir  pour calcul de probabilités
    
    
    bn_Abit=compile(as.grain(bn_Abit))
    ...
    for(k in 1:length(Time.lv))
    {
      ...
      for (j in 1:length(Scenario.lv))
      {
        ...
        Ev_Abit=setEvidence(bn_Abit,nodes=c("Time","Scenario"),states=c(as.numeric(as.character((Time.lv[k]))),Scenario.lv[j]))
        ...
        for (i in 1:length(Impacts))
        {
          ...
          marg_i=tbl_df( querygrain(Ev_Abit,nodes=Impacts[i])) %>%
    ...
    boot_Abit_qtChg[[l]]=delta_abs
   
  }
  
  
boot_Abit_qtChg_df=tbl_df(do.call(rbind,   boot_Abit_qtChg))
boot_Abit_df=tbl_df(do.call(rbind,   boot_Abit))  
write.table(boot_Abit_qtChg_df,"boot_Abit_qtChg_df.csv",sep=";",row.names = FALSE)
write.table(boot_Abit_df,"boot_Abit_df.csv",sep=";",row.names = FALSE)
